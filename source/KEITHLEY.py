...
cmds['get'] = { 'ID' :        { 'cmd' : "*IDN?", 'vital' : True },                                      #@Get device ID
                'INTERLOCK' : { 'cmd' : "SYST:INT?", 'isOK' : ['1'] , 'isNOT' : ['0'], 'vital' : True },#@Check interlock status, if isOK is returned then OK 
...
class KEITHLEY():
    ...
    def pre(self,com):
        ...
        #Set internal VSource LO to Ammeter LO connection
        self.write(com,"SOUR:VOLT:MCON ON")
        
        #Crosscheck internal connection
        internal_connection = self.read(com,"SOUR:VOLT:MCON?")
        if int(internal_connection) == 1:
            message += "Meter LO is connected to VSource LO."
        elif int(internal_connection) == 0:           
                message += "Meter LO is not internally connected to VSource LO!!! Terminating..."
                status = "e"
        else:
            message += "Meter LO connection to VSource failed!!! Terminating..."
            status = "e"
